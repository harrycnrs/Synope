...
LIST_DATA1 = list(filter(lambda e: isinstance(e, int) or isinstance(e, float), LIST_DATA))
...
DATA4 = functools.reduce(lambda x, y: 1 if (int(x) == 0) else int(x) * 1 if (int(y) == 0) else int(y), LIST_DATA3)
...
CHAINE5 = "".join(list(filter(lambda e: ord('e')<=ord(e)<=ord('n') or ord('E')<=ord(e)<=ord('N'), CHAINE)))
